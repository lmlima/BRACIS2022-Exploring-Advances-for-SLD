...
# Dataset download: https://www.kaggle.com/andrewmvd/isic-2019 or https://challenge.isic-archive.com/data/
...
def cnfg():
    ...
    # Dataset variables
    _folder = 1
    _base_path = "/home/leandro/Documentos/doutorado/dados/ISIC2019/ISIC2019"
    ...
    _imgs_folder_train = os.path.join(_base_path, "ISIC_2019_Training_Input/ISIC_2019_Training_Input")
    ...
    _save_folder = "results/" + str(_comb_method) + "_" + _model_name + "_reducer_" + str(_neurons_reducer_block) + "_fold_" + str(_folder) + "_" + str(datetime.datetime.now()).replace(' ', '')
...
def main (_folder, _csv_path_train, _imgs_folder_train, _lr_init, _sched_factor, _sched_min_lr, _sched_patience,
    ...
    val_data_loader = get_data_loader (val_imgs_path, val_labels, val_meta_data, transform=ImgEvalTransform(),
                                       batch_size=_batch_size, shuf=True, num_workers=12, pin_memory=True)
    ...
    test_data_loader = get_data_loader(test_imgs_path, test_labels, test_meta_data, transform=ImgEvalTransform(),
                                       batch_size=_batch_size, shuf=False, num_workers=12, pin_memory=True)
    print("-" * 50)
